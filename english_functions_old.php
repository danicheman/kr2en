...
//find, unset, then re-add action-verb adnominals behind the verb
...
// 	print_r($adnominal_array);
	...
	foreach($osa as $key => $clause) {
		...
		if($clause['adnominal_clause']) {
			...
			for ($i = 0; $clause[$i]->type == 'adnominal verbmod' || $clause[$i]->type == 'adnominal verb'; $i++){
...
// 						$pronoun = 'they';
			...
			foreach($adnominal_verb_array as $adnominal_element) {
				if($adnominal_element->type == 'adnominal verb') {
					...
					$adnominal_element->adnom->english = conjugate_auxillary ( $pronoun, $adnominal_element->adnom->auxillary_verb, $adnominal_element->adnom->tense ) . ' ' . $adnominal_element->adnom->english;
				} 
