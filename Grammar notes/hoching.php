...
function hoching($particle_array, $object_sentence_array_for_hoching, $object_sentence_array){	
	...
	//find out how many array inside $ojext_sentence_array and store value to $num_of_array
	$num_of_array = count($object_sentence_array_for_hoching);
	...
	for($iii=0; $iii<$num_of_array; $iii++){
		...
		$word_from_sentence=$object_sentence_array[$iii]["clause"];
		...
		//alarm for removing process sucess or not
			...
			if($rule==1){
					$num_array=count($object_sentence_array);
							...
							for($a=0; $a<=$num_array; $a++){
								$numberWords=num_of_clause_words($object_sentence_array[0]);

								for($aa=0; $aa<=$numberWords; $aa++){
				...
				$rmoved_hoching = mb_ereg_replace($remove_hoching_part,$replace,$word_from_sentence);
				...
				for($i=0; $i<$numofrow3; $i++){
					...
					mb_ereg_search_init($rmoved_hoching,$adjs);
					...
					//print all what it found out from sentence it will displayed by array
						...
						if($filtered_adjs!=""){
							...
							$after_removed_adj = mb_ereg_replace($remove_part,$replace,$rmoved_hoching);
							
							//print out after removed adjective from the words 

					
							//store remain part of words
							$rmoved_hoching = $after_removed_adj;
							...
							$after_removed_adj = $rmoved_hoching;	
						}
					}
					...
					//pint out after capture last word from words
