...
// 				
// 				if (preg_match("/(으)?로$/u", $word)) echo "preg matches";
// 				
// 				if (mb_strpos($word, '로'))echo "strpos matches";
				$word = trim($word);
				
				switch(true) {
					case ( preg_match("/(으)?로$/u", $word) ) :
...
// 		echo "final sentence regs match from preg";
	...
	if(isset($my_m_verb->verb_start_pos)) {
		...
		/* At this point I assume that if the verb is a "double", the verb part
		is two words long, then, there are no adverbs infront of that second word
		*/
		foreach($verb_words as $key => $word) {
			...
			if ($my_m_verb->isdouble && $key == 0) {
				//the first word of "verb words" is part of the verb
 				$sentence_array[$i][$key]->type = "verb word";			
				if(empty($sentence_array[$i][$key]->english))$sentence_array[$i][$key]->english = true;
 			} elseif ( $my_m_verb->iv_particle && gettype($my_m_verb->adverb) == 'array' && $adverb = array_pop($my_m_verb->adverb) ) {
				//we were able to remove an adverb, add it to this verb
				if($adverb['type'] == 'underived') {
					$sentence_array[$i][$key]->english = $adverb['english'];
					$sentence_array[$i][$key]->type = 'underived adverb';
				}else{
					//is it a noun or a verb?
					$sentence_array[$i][$key]->english = $adverb['noun']?$adverb['noun']:$adverb['with_ro'];
					$sentence_array[$i][$key]->type = 'derived adverb';
				}
			} elseif($iv_particle && $word == "잘" || $word == "못" || $word == "안" ||$word == "꼭" || $word == "다시") {
				$sentence_array[$i][$key]->type = "underived adverb";
				$sentence_array[$i][$key]->english = true;
			} else {
				$sentence_array[$i][$key]->type = "verb word";
			}
